...
public class GachaController {
    ...
    private void handleDraw(ActionEvent event) {
        ...
        KeyFrame keyFrame = new KeyFrame(Duration.seconds(0.04), e -> {
            ...
            if (lblResultDisplay != null) {
                lblResultDisplay.setText("🌀 命運之輪高速運轉中 ➔ " + fakeMsg);
            }
        });
    ...
    private void executeActualGachaLogic(Company company) {
        ...
        // 5. 抽中之後，立刻去主控制器拔掉紅點、鎖上大金庫冷卻
        mainController.setGachaUsedToday();

        // 🎯 核心修正：將 UI 阻斷型彈窗移出動畫渲染處理週期，避免 IllegalStateException
        javafx.application.Platform.runLater(() -> {
            // 6. 渲染阻斷型炫彩對話盒
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            String dialogTitle = "🎲 機會命運揭曉";
            String dialogHeader = "✨ 集團運勢解析：【" + pulledEvent.title + "】";
            String dialogContent = pulledEvent.description + "\n\n" +
                    "📊 集團財務部核算影響：\n" +
            ...
            if (pulledEvent.type.equals("傳說") && pulledEvent.cashImpact >= 10000000) {
                ...
                alert.getDialogPane().setStyle("-fx-border-color: #f1c40f; -fx-border-width: 4px; -fx-background-color: #fffde7;");
            } else if (pulledEvent.type.equals("命運") && pulledEvent.cashImpact <= -5000000) {
                alert = new Alert(Alert.AlertType.ERROR);
                dialogTitle = "🚨 💥 SSR 毀滅級黑天鵝！";
                alert.getDialogPane().setStyle("-fx-border-color: #c0392b; -fx-border-width: 4px; -fx-background-color: #fdf2f2;");
            } else if (pulledEvent.type.equals("機運")) {
                dialogTitle = "🔮 SR 業界創新奇蹟！";
                alert.getDialogPane().setStyle("-fx-border-color: #9b59b6; -fx-border-width: 3px; -fx-background-color: #faf5ff;");
            } else if (pulledEvent.cashImpact < 0) {
                // 如果是賠錢的普通命運事件，切換為 WARNING 樣式
    ...
    private void initializeEventPool() {
            ...
            eventPool.add(new GachaEvent("傳說", "全天下起鈔票雨", "地圖上觸發了瘋狂撒錢活動，總務部全員出動拿水桶去頂樓接錢！", 18000000, 0.10));
